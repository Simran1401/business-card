...
class ComInfoForms(forms.ModelForm):
    ...
    class Meta:
        model = ComInfo
        exclude = ('card_id','country','state','city')
...
class BankDetailForms(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            # 'IBAN_number': forms.TextInput(attrs={'type': 'number'}),
            # 'SWIFT_code': forms.TextInput(attrs={'type': 'number'}),
            'payment_number': forms.TextInput(attrs={'type': 'number'}),
        }
...
class SettingForms(forms.ModelForm):
    class Meta:
        model = Setting
        fields = ('view_count','about_us_section','prodcut_and_service_section','payment_option_section','gallery_section','video_section','enquiry_section')
        # exclude = ('card_id',)
